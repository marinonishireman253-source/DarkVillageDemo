...
- 当前 UI 已经能支撑原型流程，但主要依赖 `OnGUI` 直接绘制
...
- 当前主要 UI 组件包括:
  ...
  - `RuntimeDiagnostic`
- 以上大多数 UI 使用 `OnGUI` 即时绘制
- UI 当前没有统一的 `Canvas` 层级、没有 prefab 化、没有统一主题配置
- 现有 UI 视觉上已经存在稳定共性:
  - 深色半透明背景
  - 暖金色强调色
  - 偏克制的文字信息密度
...
- 现有 UI 很适合原型验证，因为迭代快、依赖少
- 但 `OnGUI` 方案不适合后续正式功能扩展
- 当前最宝贵的不是具体实现方式，而是已经形成的视觉语气和空间让位原则
